...
private:
	void operator()();
public:
	Downloader(const std::string& file_path, const std::string& dest, const std::string& searchStart, const::std::string& searchEnd ,const int useThreads = 1 , const std::string& frontComponent = "", const std::string& backComponent = "", const Naming naming = Naming::SameAsURL, const std::string& name_last = "", const int count = 1);
	~Downloader();
public:
